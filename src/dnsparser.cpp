...
{
   if ( !m_data.isValid() )
      throw EError( EError::Warning, "Parser::parseHeader() - data is invalid" );
...
ResourceRecord* Parser::parseA()
{
   ...
   {
      EString msg;
      msg.format( "Parser::parseA() - invalid RDLENGTH [%d] expected [%d]",
...
ResourceRecord* Parser::parseAAAA()
{
   ...
   {
      EString msg;
      msg.format( "Parser::parseAAAA() - invalid RDLENGTH [%d] expected [%d]",
         m_rdlength, sizeof(address) );
      throw EError( EError::Warning, msg );
   }
...
void Parser::parseDomainName( EString &dn )
{
   ...
   while ( *ptr )
   {
      if ( (val = LABEL_LENGTH(ptr)) != -1 )
      {
         ...
         if ( !m_data.validateLength(currOfs,val) )
            throw EError( EError::Warning, "label extends beyond end of message" );
      ...
      {
         EString msg;
         msg.format( "Unexpected label/offset field %x at offset %d",
...
void Parser::parseCharacterString( EString &cs )
{
