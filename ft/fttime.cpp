...
static cpStr g_day_names_short[] = {
    "Sun",
    "Mon",
    "Tue",
    "Wed",
...
static cpStr g_mnth_names_short[] = {
    "Jan",
    "Feb",
    "Mar",
    "Apr",
...
size_t format_time_into_string(pStr st, size_t max_limit, cpStr type_format,
                               struct tm *time_f, const struct timeval *tval)
{
   ...
   if (NULL == type_format)
   {
      ptr = format_time_into_specs("%c", time_f, (pStr)st, st + max_limit, tval);
   }
   else
   {
...
pStr format_time_into_specs(cpStr time_format, const struct tm *t, pStr dest,
                            cpStr max_limit, const struct timeval *p_timeval)
{
   Int tim_sec = 0;
   int i = 0;
...
pStr convert_date_time_format(const Int n, cpStr date_time_format,
                              pStr dest_buffer, cpStr max_dest_limit)
{
   Char buffer[32] = {0};
...
pStr add_timeformat_to_string(cpStr buffer, pStr dest_buffer,
...
static bool validate_date(UInt day, UInt month, UInt year)
{
   if (month < 1 || month > 12)
   {
      return false;
...
static bool initiate_and_create_date(parse_date *pd, struct tm *time_ptr,
                                     UInt date, UInt date_order, int day, int month, int year)
{
   UInt day_count = 0;
   UInt day_try = 0;
...
static bool determine_date(parse_date *pd, UInt date,
                           UInt seek, struct tm *timer_ptr)
{
...
bool DateFromStr(cpStr strIn, struct timeval *ptvout, bool isLocal)
{
   ...
   static const UChar ParseDateMonths[] = {
   ...
   while (*strIn)
   {
      ...
      if (isdigit(*strIn))
      {
         dp.field_values[dp.field_count] = strtoul(strIn, (pStr *)&strIn, 10);
         dp.field_count++;
         strIn--;
      }
   ...
   struct tm t;
   Int milliseconds;
   UInt dwOffset = 0; /* Start of date fields in dp.field_values */
...
void FTTime::Format(FTString &dest, cpStr fmt, Bool local)
{
   Char buf[2048];
...
void FTTime::Format(pStr dest, Int maxsize, cpStr fmt, Bool local)
{
   struct tm ts;
...
void FTTime::getNTPTime(ntp_time_t &ntp) const
{
   ntp.second = m_time.tv_sec + 0x83AA7E80;
   ntp.fraction = (UInt)((double)(m_time.tv_usec + 1) * (double)(1LL << 32) * 1.0e-6);
}

void FTTime::setNTPTime(const ntp_time_t &ntp)
{
   m_time.tv_sec = ntp.second - 0x83AA7E80;
   m_time.tv_usec = (UInt)((double)ntp.fraction * 1.0e6 / (double)(1LL << 32));
}
