...
FTCircularBuffer::FTCircularBuffer(Int capacity)
{
    m_capacity = capacity;
    m_data = NULL;
...
FTCircularBuffer::~FTCircularBuffer()
{
    if (m_data)
    {
        delete [] m_data;
...
Void FTCircularBuffer::initialize()
{
    m_head = 0;
    m_tail = 0;
    m_used = 0;
...
Int FTCircularBuffer::readData(pUChar dest, Int offset, Int length, Bool peek)
{
...
void FTCircularBuffer::writeData(pUChar src, Int offset, int length)
{
    FTMutexLock lockMutex(m_mutex);
...
void FTCircularBuffer::modifyData(pUChar src, Int offset, int length)
{
    FTMutexLock lockMutex(m_mutex);
    ...
    if (offset + length > used())
    {
        ...
        FTCircularBufferError_AttemptToModifyDataOutsideBoundsOfCurrentBuffer *e =
            new FTCircularBufferError_AttemptToModifyDataOutsideBoundsOfCurrentBuffer();
