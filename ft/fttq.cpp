                                    ...
                                    FTThreadQueueBase::Mode eMode)
{
    ...
    else
    {
        ...
        semFree().init(semFreeId());
        semMsgs().init(semMsgsId());
#elif defined(FT_GCC)
...
Void FTThreadQueuePublic::allocDataSpace(cpStr sFile, Char cId, Int nSize)
{
    ...
    m_pData = (FTThreadMessage*)
        (((pChar)m_sharedmem.getDataPtr()) + sizeof(ftthreadmessagequeue_ctrl_t));
}
