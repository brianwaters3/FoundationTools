...
#if defined(WIN32) || defined(WIN64) || defined(SOLARIS_RAW) || (__GNUC__ == 2) || defined(__HP_aCC)
...
public:
    ...
    inline FTGetOpt();
    inline FTGetOpt(const FTGetOpt&);
    inline FTGetOpt(const int argc_, char** argv_, 
                  const char* FieldSeparator=0x0);
    inline FTGetOpt(const char* FileName, 
                  const char* CommentStart=0x0, const char* CommentEnd=0x0,
                  const char* FieldSeparator=0x0);
    inline ~FTGetOpt();
    inline FTGetOpt& operator=(const FTGetOpt&);


    // (*) absorbing contents of another FTGetOpt object
    inline void            absorb(const FTGetOpt& That);
    ...
    //     -- setting variables
    //                  i) from outside of FTGetOpt (considering prefix etc.)
...
private:
    ...
    //        this container makes them 'available' until the getopt object is destroyed.
    ...
    // (*) helper functions ----------------------------------------------------
    //                  set variable from inside FTGetOpt (no prefix considered)
    ...
    const FTGetOpt::variable*   __DBE_get_variable(const std::string str);
    ...
    std::string  __double2string(const double& Value) const {
        ...
        ft_sprintf_s(tmp, sizeof(char)*128, "%e", Value);
    ...
    std::string  __int2string(const int& Value) const {
        ...
        char* tmp = new char[128];
        ft_sprintf_s(tmp, sizeof(char)*128, "%i", Value);
        std::string result(tmp);
        delete [] tmp;
        return result;
    }
...
//
inline void
FTGetOpt::__basic_initialization()
{
...
inline
FTGetOpt::FTGetOpt(const int argc_, char ** argv_, 
               const char* FieldSeparator /* =0x0 */)     
...
inline
FTGetOpt::FTGetOpt(const char* FileName,
               const char* CommentStart  /* = 0x0 */, const char* CommentEnd /* = 0x0 */,
               const char* FieldSeparator/* = 0x0 */)     
{
    __basic_initialization();
...
inline
FTGetOpt::FTGetOpt(const FTGetOpt& That)
{ FTGetOpt::operator=(That); }
...
inline FTGetOpt&
FTGetOpt::operator=(const FTGetOpt& That)
{
...
inline void
FTGetOpt::absorb(const FTGetOpt& That)
{
    if (&That == this) return;
    ...
    // strip the section headers if they exist
    STRING_VECTOR::iterator it = __tmp.begin();
    section = "";
    for (; it != __tmp.end(); it++)
    {
        std::string arg = *it;
        ...
        if( arg.length() > 1 && arg[0] == '[' && arg[arg.length()-1] == ']' ) {
            section = __DBE_expand_string(arg.substr(1, arg.length()-2));
        }
        else if (section.length() > 0)
        {
            unsigned int ofs;
            for (ofs=0; ofs < section.length() && ofs < arg.length(); ofs++)
            {
                if (section[ofs] != arg[ofs])
                    break;
            }
            if (ofs == section.length() && arg[ofs++] == '/') // arg starts with section
                *it = arg.substr(ofs, arg.length() - ofs);
        }
    }    
...
inline void
FTGetOpt::__parse_argument_vector(const STRING_VECTOR& ARGV)
{
    ...
    if( (*it).length() == 1 || (*it)[0] != '[' || (*it)[(*it).length()-1] != ']' ) {
        argv.push_back(*it);
        ++it;
    }
...
FTGetOpt::__read_in_file(const char* FileName)
{
...
inline STRING_VECTOR
FTGetOpt::__read_in_stream(std::istream& istr)
{
...
inline void
FTGetOpt::__skip_whitespace(std::istream& istr)
...
FTGetOpt::__get_next_token(std::istream& istr)
...
inline const std::string
FTGetOpt::__get_string(std::istream& istr)
...
inline const std::string
FTGetOpt::__get_until_closing_bracket(std::istream& istr)
    // parse input until next matching }
{
    std::string str = "";
    int    tmp = 0;
...
inline std::string
FTGetOpt::__process_section_label(const std::string& Section,
                                STRING_VECTOR& section_stack)
{
...
inline double
FTGetOpt::__convert_to_type(const std::string& String, double Default) const
{
    double tmp;
    if( ft_sscanf_s(String.c_str(),"%lf", &tmp) != 1 ) return Default;
    return tmp;
}

// convert string to INT, if not possible return Default
inline int
FTGetOpt::__convert_to_type(const std::string& String, int Default) const
{
...
//.............................................................................
inline const std::string
FTGetOpt::__get_remaining_string(const std::string& String, const std::string& Start) const
...
inline bool
FTGetOpt::search(const char* Option)
{    
...
inline bool
FTGetOpt::search(unsigned No, const char* P, ...)
{
...
inline void
FTGetOpt::reset_cursor()
{ search_failed_f = false; cursor = 0; }

inline void
FTGetOpt::init_multiple_occurrence()
...
//
inline const std::string
FTGetOpt::operator[](unsigned idx) const
{ return idx < argv.size() ? argv[idx] : ""; }

inline int
FTGetOpt::get(unsigned Idx, int Default) const
{
...
inline double
FTGetOpt::get(unsigned Idx, const double& Default) const
{
    if( Idx >= argv.size() ) return Default;
    return __convert_to_type(argv[Idx], Default);
}
...
inline const std::string
FTGetOpt::get(unsigned Idx, const char* Default) const
{
    if( Idx >= argv.size() ) return Default;
...
inline double
FTGetOpt::next(const double& Default)
{
    if( search_failed_f ) return Default;
    cursor++;
...
inline const std::string
FTGetOpt::next(const char* Default)
{
    using namespace std;
    ...
    ft_strncpy_s(result, Remain.length()+1, Remain.c_str(), Remain.length()+1);
...
inline int
FTGetOpt::follow(int Default, const char* Option)
{
...
inline double
FTGetOpt::follow(const double& Default, const char* Option)
{
    // (*) record requested of argument is entirely handled in 'search()' and 'next()'
    if( search(Option) == false ) return Default;
    return next(Default);
...
inline const std::string
FTGetOpt::follow(const char* Default, const char* Option)
{
    // (*) record requested of argument is entirely handled in 'search()' and 'next()'
    if( search(Option) == false ) return Default;
    return next(Default);
...
inline int
FTGetOpt::follow(int Default, unsigned No, const char* P, ...)
{
    // (*) record requested of argument is entirely handled in 'search()' and 'next()'
...
inline double
FTGetOpt::follow(const double& Default, unsigned No, const char* P, ...)
{
    // (*) record requested of argument is entirely handled in 'search()' and 'next()'
    if( No == 0 ) return Default;
    if( search(P) == true ) return next(Default);
...
inline const std::string
FTGetOpt::follow(const char* Default, unsigned No, const char* P, ...)
{
    // (*) record requested of argument is entirely handled in 'search()' and 'next()'
    if( No == 0 ) return Default;
    if( search(P) == true ) return next(Default);
...
FTGetOpt::nominus_followers(const char* Option)
{
    ...
    while( 1 + 1 == 2 ) {
        ++cursor;
        if( cursor >= argv.size() ) { 
            cursor = (unsigned int)argv.size() - 1;
            return result_list;
        }
...
inline std::vector<std::string> 
FTGetOpt::nominus_followers(unsigned No, ...)
{
    std::vector<std::string>  result_list;
...
//
inline int
FTGetOpt::direct_follow(int Default, const char* Option)
{
...
inline double
FTGetOpt::direct_follow(const double& Default, const char* Option)
{
    const char* FollowStr = __match_starting_string(Option);
...
inline const std::string
FTGetOpt::direct_follow(const char* Default, const char* Option)
{
    if( search_failed_f ) return Default;
    const char* FollowStr = __match_starting_string(Option);
    if( FollowStr == 0 )  return Default;
...
inline std::vector<std::string>
FTGetOpt::string_tails(const char* StartString)
{
...
FTGetOpt::int_tails(const char* StartString, const int Default /* = -1 */)
{
...
FTGetOpt::double_tails(const char*  StartString, 
...
inline const char*
FTGetOpt::__match_starting_string(const char* StartString)
...
//
inline bool
FTGetOpt::options_contain(const char* FlagList) const
{
...
inline bool
FTGetOpt::argument_contains(unsigned Idx, const char* FlagList) const
{
    if( Idx >= argv.size() ) return false;

    // (*) record requested of argument for later ufo-detection
    //     an argument that is checked for flags is considered to be 'requested'
    ((FTGetOpt*)this)->__record_argument_request(argv[Idx]);
...
inline bool
FTGetOpt::__check_flags(const std::string& Str, const char* FlagList) const
{
    ...
    // return vector of nominus arguments
{
    ...
    for(; it != idx_nominus.end(); ++it) {
        ...
        //     tagged as 'requested'
        ((FTGetOpt*)this)->__record_argument_request(argv[*it]);
    }
    return nv;
}

inline std::string
FTGetOpt::next_nominus()
{
...
//
inline int
FTGetOpt::operator()(const char* VarName, int Default) const
{
...
inline double
FTGetOpt::operator()(const char* VarName, const double& Default) const
{
    // (*) recording of requested variables happens in '__find_variable()'
    const variable*  sv = __find_variable(VarName);
    if( sv == 0 ) return Default;
...
inline const std::string
FTGetOpt::operator()(const char* VarName, const char* Default) const
{
    // (*) recording of requested variables happens in '__find_variable()'
    const variable*  sv = __find_variable(VarName);
    if( sv == 0 ) return Default;
...
inline int
FTGetOpt::operator()(const char* VarName, int Default, unsigned Idx) const
{
    // (*) recording of requested variables happens in '__find_variable()'
    const variable* sv = __find_variable(VarName);
    if( sv == 0 ) return Default;
...
inline double
FTGetOpt::operator()(const char* VarName, const double& Default, unsigned Idx) const
{
    // (*) recording of requested variables happens in '__find_variable()'
    const variable* sv = __find_variable(VarName);
    if( sv == 0 ) return Default;
...
inline const std::string
FTGetOpt::operator()(const char* VarName, const char* Default, unsigned Idx) const
{
    // (*) recording of requested variables happens in '__find_variable()'
    const variable*  sv = __find_variable(VarName);
    if( sv == 0 ) return Default;
...
inline void 
FTGetOpt::__record_argument_request(const std::string& Name)
{
...
inline void 
FTGetOpt::__record_variable_request(const std::string& Name)
{
    if( ! __request_recording_f ) return; 
...
// (*) following functions are to be used from 'outside', after getopt has parsed its
//     arguments => append an argument in the argument vector that reflects the addition
inline void
FTGetOpt::__set_variable(const char* VarName, const char* Value)
{
    const FTGetOpt::variable* Var = __find_variable(VarName);
    if( Var == 0 ) variables.push_back(variable(VarName, Value, _field_separator.c_str()));
    else           ((FTGetOpt::variable*)Var)->take(Value, _field_separator.c_str());    
}

inline void
FTGetOpt::set(const char* VarName, const char* Value, const bool Requested /* = yes */)
{     
...
inline void
FTGetOpt::set(const char* VarName, const double& Value, const bool Requested /* = yes */)
{ __set_variable(VarName, __double2string(Value).c_str()); }

inline void 
FTGetOpt::set(const char* VarName, const int Value, const bool Requested /* = yes */)
{ __set_variable(VarName, __int2string(Value).c_str()); }


inline unsigned
FTGetOpt::vector_variable_size(const char* VarName) const
{
    const variable*  sv = __find_variable(VarName);
    if( sv == 0 ) return 0;
...
inline STRING_VECTOR
FTGetOpt::get_variable_names() const
{
    STRING_VECTOR  result;
    std::vector<FTGetOpt::variable>::const_iterator it = variables.begin();
...
inline STRING_VECTOR
FTGetOpt::get_section_names() const
{ return section_list; }

inline const FTGetOpt::variable*
FTGetOpt::__find_variable(const char* VarName) const
{
    const std::string Name = prefix + VarName;

    // (*) record requested variable for later ufo detection
    ((FTGetOpt*)this)->__record_variable_request(Name);
...
//---------------------------------------------------------------------------
inline std::string
FTGetOpt::__DBE_expand_string(const std::string str)
{
...
inline STRING_VECTOR
FTGetOpt::__DBE_get_expr_list(const std::string str_, const unsigned ExpectedNumber)
...
inline const FTGetOpt::variable*
FTGetOpt::__DBE_get_variable(std::string VarName)
{
    static FTGetOpt::variable ev;
    std::string secure_Prefix = prefix;

    prefix = section;
    // (1) first search in currently active section
    const FTGetOpt::variable* var = __find_variable(VarName.c_str());
    ...
    ft_sprintf_s(tmp, sizeof(char)*(VarName.length() + 25), 
...
inline std::string
FTGetOpt::__DBE_expand(const std::string expr)
{
    // ${: } pure text
    if( expr[0] == ':' )
    ...
    // ${! expr} maxro expansion
        ...
        const FTGetOpt::variable* Var = __DBE_get_variable(expr.substr(1));
        // error
    ...
    // ${@ } - vector subscription
        ...
        STRING_VECTOR          A   = __DBE_get_expr_list(expr.substr(1), 2);
        const FTGetOpt::variable* Var = __DBE_get_variable(A[0]);
        // error
        if( Var->name == "" ) {
    ...
    const FTGetOpt::variable* B = __DBE_get_variable(A[0]);
...
//
inline bool
FTGetOpt::__search_string_vector(const STRING_VECTOR& VecStr, const std::string& Str) const
{
...
inline STRING_VECTOR
FTGetOpt::unidentified_arguments(unsigned Number,
...
inline STRING_VECTOR
FTGetOpt::unidentified_arguments() const
{ return unidentified_arguments(_requested_arguments); }

inline STRING_VECTOR
FTGetOpt::unidentified_arguments(const STRING_VECTOR& Knowns) const
{
    STRING_VECTOR ufos;
    STRING_VECTOR::const_iterator it = argv.begin();
...
inline STRING_VECTOR
FTGetOpt::unidentified_options(unsigned Number,
...
inline STRING_VECTOR
FTGetOpt::unidentified_options() const
{ 
...
inline STRING_VECTOR
FTGetOpt::unidentified_options(const STRING_VECTOR& Knowns) const
{
    STRING_VECTOR ufos;
    STRING_VECTOR::const_iterator it = argv.begin();
    ++it; // forget about argv[0] (application or filename)
...
inline std::string
FTGetOpt::unidentified_flags(const char* KnownFlagList, int ArgumentNumber=-1) const
    // Two modes:
...
inline STRING_VECTOR
FTGetOpt::unidentified_variables(unsigned Number,
...
inline STRING_VECTOR
FTGetOpt::unidentified_variables(const STRING_VECTOR& Knowns) const
{
    STRING_VECTOR ufos;

    victorate(FTGetOpt::variable, variables, it) {
...
inline STRING_VECTOR
FTGetOpt::unidentified_variables() const
{  return unidentified_variables(_requested_variables); }


inline STRING_VECTOR
FTGetOpt::unidentified_sections(unsigned Number,
...
inline STRING_VECTOR
FTGetOpt::unidentified_sections() const
{ return unidentified_sections(_requested_sections); }

inline STRING_VECTOR
FTGetOpt::unidentified_sections(const STRING_VECTOR& Knowns) const
{
    STRING_VECTOR ufos;
...
inline STRING_VECTOR
FTGetOpt::unidentified_nominuses(unsigned Number, const char* Known, ...) const
{
...
inline STRING_VECTOR
FTGetOpt::unidentified_nominuses() const {
...
inline STRING_VECTOR
FTGetOpt::unidentified_nominuses(const STRING_VECTOR& Knowns) const
{
    STRING_VECTOR ufos;
...
//
inline
FTGetOpt::variable::variable()
{}

inline
FTGetOpt::variable::variable(const variable& That)
{
#ifdef WIN32
    operator=(That);
#else
    FTGetOpt::variable::operator=(That);
#endif
}


inline
FTGetOpt::variable::variable(const char* Name, const char* Value, const char* FieldSeparator)
    : name(Name)
{
...
inline const std::string*
FTGetOpt::variable::get_element(unsigned Idx) const
{ if( Idx >= value.size() ) return 0; else return &(value[Idx]); }

inline void
FTGetOpt::variable::take(const char* Value, const char* FieldSeparator)
{
    using namespace std;
    ...
    ft_strcpy_s(copy, strlen(Value)+1, Value);
...
inline FTGetOpt::variable&
FTGetOpt::variable::operator=(const FTGetOpt::variable& That)
{
    if( &That != this) {
        name     = That.name;
        value    = That.value;
